...
df = pd.read_csv("UPDATED_PRSA_Data_Aotizhongxin_20130301-20170228.csv", index_col=None)
...
# data_monthly = df.iloc[:,1:-3].resample('M').mean()
# fig, ax = plt.subplots(nrows=num_features, ncols=1, figsize=(10,2*num_features))
# for i, feature in enumerate(features1): 
#     ax[i].plot(data_monthly.index, data_monthly[feature], label=feature)
#     ax[i].set_ylabel(feature)
#     ax[i].legend()
...
st.write("dataset berikut merupakan yang ter-update setelah dilakukan data cleaning.")
st.dataframe(df)

st.header("Menampilkan statistik")
st.dataframe(df.describe())
st.header("Menampilkan Korelasi")
st.dataframe(df.iloc[:, 1:-4].corr())
st.write("Terlihat pada matriks korelasi bahwa semua fitur memilki korelasi, baik korelasi postitif maupun negatif")
...
st.pyplot(fig)
st.write("**datetime tidak dapat dijadikan sebagai index, dan fitur date tidak dapat digunakan sebagai variabel x dalam grafik pada streamlit. Untuk dapat memahami grafik dengan lebih jelas dapat dilihat pada file .ipynb (https://github.com/RofidRamdhan/cobastreamlit.git)")
